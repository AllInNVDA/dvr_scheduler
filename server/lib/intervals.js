...
function pushInterval(from, to) {
  ...
  this._intervals.push(new Interval(from, to,true));
  return Interval.prototype.id-1;
}
...
function pushArray(from, to, validate) {
  ...
  for(var i = 0; i < from.length; i++) {
    this._intervals.push(new Interval(from[i], to[i],true));
  }
}
